...
//funzionante da rifinire
/*

- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath;
{
    NSString *textTitolo = [[avvisi objectAtIndexPath:indexPath] titolo];
    NSString *text = [[avvisi objectAtIndexPath:indexPath] descrizione];
    
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:@"EEEE dd"];
    
    NSString *textData = [formatter stringFromDate:[[avvisi objectAtIndexPath:indexPath] datacreazione]];
    
    
    CGSize constraint = CGSizeMake(CELL_CONTENT_WIDTH - (CELL_CONTENT_MARGIN * 2), 20000.0f);
    
    CGSize sizeTitolo = [textTitolo sizeWithFont:[UIFont systemFontOfSize:FONT_SIZETITLE] constrainedToSize:constraint lineBreakMode:NSLineBreakByWordWrapping];
    ...
    CGFloat height = MAX((size.height+sizeTitolo.height+sizeData.height+50+CELL_CONTENT_MARGIN), 44.0f);
...
- (UITableViewCell *)tableView:(UITableView *)tv cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UILabel *labelDescrizione = nil;
    UILabel *labelTitolo = nil;
     UILabel *labelData = nil;
    static NSString *CellIdentifier = @"CellAvvisi";
    ...
    if (cell == nil)
    {
        ...
        labelDescrizione = [[UILabel alloc] initWithFrame:CGRectMake(labelTitolo.bounds.origin.x, labelTitolo.bounds.size.height, 0, 0)];
        [labelDescrizione setLineBreakMode:NSLineBreakByWordWrapping];
        [labelDescrizione setNumberOfLines:0];
        [labelDescrizione setFont:[UIFont systemFontOfSize:FONT_SIZE]];
        [labelDescrizione setTag:3];
      
        [[cell contentView] addSubview:labelDescrizione];
    ...
    if (!labelTitolo)
        labelTitolo = (UILabel*)[cell viewWithTag:1];
    
    [labelTitolo setText:textTitolo];
    [labelTitolo setFrame:CGRectMake(CELL_CONTENT_MARGIN, CELL_CONTENT_MARGIN, CELL_CONTENT_WIDTH - (CELL_CONTENT_MARGIN * 2), MAX(sizeTitolo.height, 44.0f))];
    
    if (!labelData)
        labelData = (UILabel*)[cell viewWithTag:2];
    
    [labelData setText:textData];
    [labelData setFrame:CGRectMake(CELL_CONTENT_MARGIN, CELL_CONTENT_MARGIN+labelTitolo.bounds.size.height, CELL_CONTENT_WIDTH - (CELL_CONTENT_MARGIN * 2), MAX(sizeData.height, 44.0f))];
    
    if (!labelDescrizione)
        labelDescrizione = (UILabel*)[cell viewWithTag:3];
    
    [labelDescrizione setText:text];
    [labelDescrizione setFrame:CGRectMake(CELL_CONTENT_MARGIN, CELL_CONTENT_MARGIN+labelTitolo.bounds.size.height + labelData.bounds.size.height, CELL_CONTENT_WIDTH - (CELL_CONTENT_MARGIN * 2), MAX(size.height, 44.0f))];
